...
export const createInvoice = async (req, res) => {
    try {
        ...
        const { client, products, dueDate, paymentType } = parsedBody;
        ...
        if (error.name === 'ZodError') {
            return res.status(400).json({ message: "Błąd walidacji danych", errors: error.errors });
        }
...
export const invoicePDF = async (req, res) => {
    try {
        ...
        console.log("Error generating invoice PDF", e);
        res.status(500).json({ message: "Internal server error", error: e.message });
    }
}